...
| `/api/jobs/:id/stream` | GET (SSE) | 推送 job heartbeat、单资源状态更新，供前台 UI 实时显示；回退到轮询时可不调用。|
...
```json
{
  ...
  "relativeProfile": "clips-default",
  "renameStrategy": "keep-extension",
  "maxConcurrency": 3
}
```

`relativeProfile` 是插件在配置页面中选择的白名单别名，Server 会在启动配置里维护 `profile -> [allowedDirs]` 映射。请求体只能引用已存在的 profile，无法动态扩展白名单，PathGuard 会据此校验 `saveDir/docPath` 是否在允许范围内。
...
- **下载去重**：新增 InFlightRegistry（`urlHash -> Promise<DownloadResult>`）。当某个 URL 正在下载时，后续 job 会挂起等待结果，而不是再次入队下载；完成后统一写入缓存并唤醒等待者。
...
- JobStore 使用轻量 append-only 日志 + snapshot，将 job 元数据、result 列表、最新 heartbeat 持久化；server 重启可恢复 job 状态并继续复制/下载或给出失败提示。
...
- 内存 Map + 持久化 JSON 快照（强制）。每次 `update/delete` 都写入 append-only 日志，后台周期性压缩为 snapshot，确保重启后仍能命中历史缓存。
- API：`lookup(urlHash)`, `update(urlHash, path)`, `delete(urlHash)`。
- 启动时先 replay 日志，再加载 snapshot；当路径不存在时自动清理该条并写入日志。
- 维护 `maxEntries`，超过时按 LRU 淘汰，并把 LRU 删除动作同步到日志，保证多进程一致性。
...
- `handleCacheHitDifferentPath`：优先尝试 APFS clone/硬链接，不支持时再复制 canonicalPath → targetPath，然后更新缓存。
...
- 校验 `saveDir` 是否位于 server 配置的 profile 白名单内；插件只能传递 `relativeProfile` 名称，无法新增目录；模块会进一步检测 `..`、符号链接逃逸。
- 验证 filename 中不包含路径分隔符/控制字符。

### 8. InFlightRegistry

- 保存 `urlHash -> {promise, requesters}` 映射；QueueManager 在调度前会查询该 registry。
- 第一个请求发起真实下载，并把 promise 注入 registry；后续同 URL 请求只需 `await` 该 promise，避免重复走网络。
- 下载完成或失败都会触发 `finally` 清理，必要时写入错误信息供等待者感知，再决定是否重新排队。
...
| 模块 | 用例 |
|------|------|
| JobService | 创建/查询/取消、崩溃后日志 replay、heartbeat 更新 |
| QueueManager | 全局/host 并发限制、节流、优先级、queueReason 输出 |
| CacheIndex | 命中、更新、路径失效自动清理、LRU 淘汰、日志+快照恢复 |
...
| FetchWorker | 成功下载、重定向、429 退避、超时重试、TLS 错误、in-flight 去重配合 |
| API | POST 创建任务（永远异步）、GET 查询心跳、取消接口、失败结果格式 |
| 集成 | 使用 a.md URL：模拟 12 张图片，覆盖缓存命中、复制/硬链接、失败重试、SSE 推送、`relativePathFromDoc` 正确性 |
...
3. **轮询策略**：
   ...
   - Server 额外提供 `/api/jobs/:id/stream`（SSE）用于实时推送心跳与进度，插件在前台活跃时可以改用 SSE 减少无效轮询，后台或网络受限时再退回轮询模式。
4. **结果应用**：
...
6. **配置项**：`serverUrl`, `token`, `defaultSaveDir`, `defaultRelativeRoot`, `relativeProfile`, `maxSyncItems`, `pollIntervalMin/Max`（`relativeProfile` 只保存 profile 名称，白名单本身仅在 server 侧配置）。
...
| 第 3 阶段：缓存/任务 (1-2d) | JobService、CacheIndex、QueueManager、持久化实现 | 支持 job 生命周期、心跳与崩溃恢复 |
...
| 服务器崩溃丢任务 | 低 | JobStore/CacheIndex 均写入持久日志 + snapshot，重启后自动 replay 恢复任务与缓存 |
...
| ID | 决策点 | 选项 | 结论 | 备注 |
|----|--------|------|------|------|
| D1 | Server 返回方式 | 仅同步 / 仅异步 / 自适应 | 仅异步 | 满足“立即返回 jobId + 轮询”的硬需求，接口语义一致 |
